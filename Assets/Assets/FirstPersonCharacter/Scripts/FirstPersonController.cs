...
namespace UnitySampleAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        private void Update()
        {
			...
			_blink = CrossPlatformInputManager.GetButtonDown ("Fire2");

			if (_blink) {
				this.transform.position = this.transform.position + _camera.transform.forward*blinkDistance;
				//RaycastHit hitInfo;
				//Physics.Raycast(transform.position, transform.forward, out hitInfo);
				//Vector3 fwd = this.transform.TransformDirection(Vector3.forward);
				//this.transform.position = this.transform.position + hitInfo.transform.forward*blinkDistance;
				//Vector3 temp2 = this.transform.forward*blinkDistance;
			}
